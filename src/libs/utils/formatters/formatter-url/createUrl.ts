import { createQueryParams, TQueryParams } from './createQueryParams';
import { formatterPaths, TPath } from './formatPaths';
...
type CreateUrlFunction = (config: { baseUrl: string; basePaths?: TPath[] }) => BuildUrlFunction;
...
export const createUrl: CreateUrlFunction = ({ baseUrl, basePaths }) => {
  return ({ queryParams, endpoints } = {}) => {
    ...
    const fullPaths = formatterPaths(url.pathname.split('/'), basePaths, endpoints);
